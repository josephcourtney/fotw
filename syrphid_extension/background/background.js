...
const handleAsyncError = async (task, label) => {
    try {
        return await task();
    } catch (error) {
...
const getPlugins = () => {
    return getDetails('plugins', isFeatureAvailable(navigator.plugins)
...
const getServiceWorkers = async () => {
    return handleAsyncError(async () => {
        const serviceWorkers = 'serviceWorker' in navigator 
            ? await navigator.serviceWorker.getRegistrations().then(regs => regs.map(reg => reg.scope)) 
            : [];
        return getDetails('serviceWorkers', serviceWorkers);
    }, 'getServiceWorkers');
};

const getInstalledComponents = async () => {
    return handleAsyncError(async () => {
...
const getMultiMonitorSetup = async () => {
    return handleAsyncError(async () => {
        const screens = 'getScreenDetails' in window ? await window.getScreenDetails() : [];
...
const getDisplayInfo = async () => {
    return handleAsyncError(async () => {
        ...
        return getDetails('displayInfo', {
            ...
            availableScreenArea: { width: screen.availWidth, height: screen.availHeight },
...
const getARCapabilities = async () => {
    const arCapabilities = 'xr' in navigator && await navigator.xr.isSessionSupported('immersive-ar');
...
const getWebLocks = async () => {
    if (!('locks' in navigator)) {
        return getDetails('webLocks', 'Web Locks API not supported in this browser');
    }
...
const getDigitalInkCapabilities = async () => {
    return getDetails('digitalInkCapabilities', 'Digital Ink API requires specific permissions and availability');
};
...
const backgroundEvents = [
    // deviceEvents: [
    { name: "devicemotion", parameters: ["acceleration", "accelerationIncludingGravity", "rotationRate", "interval"] },
    { name: "deviceorientation", parameters: ["alpha", "beta", "gamma", "absolute"] },
    { name: "deviceorientationabsolute", parameters: ["alpha", "beta", "gamma", "absolute"] },
    ...
    { name: "message", parameters: ["data", "origin", "lastEventId", "source", "ports"] },
...
const logEvent = (event, parameters) => {
    const log = {
        type: event.type,
        timestamp: new Date(),
        details: parameters.reduce((acc, param) => ({ ...acc, [param]: event[param] }), {})
    };
    sendOverWebSocket('eventLog', log);
    if (event.type === 'message') event[INTERNAL_MESSAGE_FLAG] = true;
};
...
const eventManager = (() => {
    ...
    const createHandler = (eventName, parameters) => event => {
        if (eventName === 'message' && event[INTERNAL_MESSAGE_FLAG]) return;
...
const reportInitialState = async () => {
    ...
    for (const { label, task } of tasks) {
        await sendOverWebSocket(label, await handleAsyncError(task, label));
    }
};
...
const pollState = async () => {
    const tasks = [
        ...
        { label: 'Media Capabilities and WebRTC Support', task: getMediaCapabilities },
